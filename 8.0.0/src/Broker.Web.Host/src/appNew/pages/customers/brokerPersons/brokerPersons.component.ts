...
import { CityDto,CountryDto, BrokerPersonDto } from '@shared/service-proxies/service-proxies';
...
class PagedBrokerPersonsRequestDto extends PagedRequestDto {
    keyword: string;
}


@Component({
    selector: 'app-admin-brokerPersons',
    templateUrl: './brokerPersons.component.html',
    styleUrls: ['./brokerPersons.component.scss']
})
export class AdminBrokerPersonsComponent extends DataTableComponentBase {
    protected rejectResonBalance?(obj: object): void {
        throw new Error('Method not implemented.');
    }
    public closeResult: string;
    columnDefs: any[] = [];
    selectedObj: BrokerPersonDto = new BrokerPersonDto();
    searchValue: any;
            ...
            },
            {
                ...
                render: (data, type, full) => {
                    if (this.isDeleted == false) {
                        let rowDiv = /*this.isGranted("Write.Permission.Brokers") ?*/ /*'<button type="button" class="btn btn-round btn-primary buttons-html5 edit"><i class="fa fa-edit"></i></button>'*/ /*: ""*/  "";
                        //rowDiv +=  /*this.isGranted("Delete.Permission.Brokers") ?*/  '<a href="javascript:void(0);" class="btn btn-icon btn-sm mx-3 btn-danger delete confirm" data-action="Delete" data-id="' + data + '" title="' + this.l("Common.Delete") + '"><i class="fa fa-trash"></i></a>' /*: ""*/;
                        //return rowDiv;
                        //if (full.status == 0) {
                        //    rowDiv += /*this.isGranted("Write.Permission.Brokers") ?*/ '<a href="javascript:void(0);" class="btn btn-icon btn-sm mx-3 btn-success active confirm" data-action="active" data-id="' + data + '" title = "' + this.l("Common.Active") + '"> <i class="fa fa-check"></i></a>' /*: ''*/;
                        //    rowDiv += /*this.isGranted("Write.Permission.Brokers") ?*/ '<a href="javascript:void(0);" class="btn btn-icon btn-sm mx-3 btn-danger deactivate confirm" data-action="deactivate" data-id="' + data + '" title = "' + this.l("Common.Deactivate") + '"> <i class="fa fa-times"></i></a>' /*: ''*/;
                        ...
                        return this.isGranted("Delete.Permission.Brokers") ?  '<a href="javascript:void(0);" class="btn btn-icon btn-sm mx-3 btn-danger delete confirm" data-action="Restore" data-id="' + data + '" title="' + this.l("Common.Restore") + '"><i class="fa fa-undo"></i></a>' : "";
                    }
                }
            }
        ];

        initDataTableInput.DataTableSrc = "/api/services/app/BrokerPerson/IsPaged";
        this.columnDefs = super.initDataTable(initDataTableInput);

    }

    protected edit?(obj: BrokerPersonDto): void {
    ...
    addNew() {
        //this.selectCountry = [];
        //this.selectedObj = new BrokerPersonDto();
        //this.openModal();
    }

    //openModal() {
    //    this.dialogService.open(DialogManageBrokerPersonComponent, {
    //        context: {
