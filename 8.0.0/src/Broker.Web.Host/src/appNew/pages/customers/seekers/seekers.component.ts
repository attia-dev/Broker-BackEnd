...
import { CityDto,CountryDto, SeekerDto } from '@shared/service-proxies/service-proxies';
...
class PagedSeekersRequestDto extends PagedRequestDto {
    keyword: string;
}
...
export class AdminSeekersComponent extends DataTableComponentBase {
    protected rejectResonBalance?(obj: object): void {
        throw new Error('Method not implemented.');
    }
    public closeResult: string;
    columnDefs: any[] = [];
    selectedObj: SeekerDto = new SeekerDto();
    searchValue: any;
            ...
            },
            {
                ...
                render: (data, type, full) => {
                    if (this.isDeleted == false) {
                        let rowDiv = /*this.isGranted("Write.Permission.Seekers") ?*/ /*'<button type="button" class="btn btn-round btn-primary buttons-html5 edit"><i class="fa fa-edit"></i></button>'*/ /*: ""*/  "";
                        if (full.isActive == true) {
                            rowDiv += this.isGranted("Write.Permission.Seekers") ? '<a href="javascript:void(0);" class="btn btn-icon btn-sm mx-3 btn-danger deactivate confirm" data-action="deactivate" data-id="' + data + '" title = "' + this.l("Common.Deactivate") + '"> <i class="fa fa-times"></i></a>' : '';
                        }
                        else {
                            rowDiv += this.isGranted("Write.Permission.Seekers") ? '<a href="javascript:void(0);" class="btn btn-icon btn-sm mx-3 btn-success active confirm" data-action="active" data-id="' + data + '" title = "' + this.l("Common.Active") + '"> <i class="fa fa-check"></i></a>' : '';
                        }
                        rowDiv += this.isGranted("Delete.Permission.Seekers") ?  '<a href="javascript:void(0);" class="btn btn-icon btn-sm mx-3 btn-danger delete confirm" data-action="Delete" data-id="' + data + '" title="' + this.l("Common.Delete") + '"><i class="fa fa-trash"></i></a>' : "";
                        return rowDiv;
                    }
                    else {
                        return this.isGranted("Delete.Permission.Seekers") ?  '<a href="javascript:void(0);" class="btn btn-icon btn-sm mx-3 btn-danger delete confirm" data-action="Restore" data-id="' + data + '" title="' + this.l("Common.Restore") + '"><i class="fa fa-undo"></i></a>' : "";
                    }
                }
            }
        ];

        initDataTableInput.DataTableSrc = "/api/services/app/Seeker/IsPaged";
        this.columnDefs = super.initDataTable(initDataTableInput);

    }

    protected edit?(obj: SeekerDto): void {
    ...
    addNew() {
        //this.selectCountry = [];
        //this.selectedObj = new SeekerDto();
        //this.openModal();
    }

    //openModal() {
    //    this.dialogService.open(DialogManageSeekerComponent, {
    //        context: {
