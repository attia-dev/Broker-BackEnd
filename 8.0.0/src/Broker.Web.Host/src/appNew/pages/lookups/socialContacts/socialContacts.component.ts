...
import { ContactUsDto, CountryDto, DefinitionDto, SocialContactDto } from '@shared/service-proxies/service-proxies';
import { PagedListingComponentBase, PagedRequestDto } from '@shared/paged-listing-component-base';
import { finalize } from 'rxjs/operators';
import { NbDialogService } from '@nebular/theme';
import { DialogManageSocialContactComponent } from './dialog-manage-socialContact.component';
...
@Component({
    selector: 'app-admin-socialContacts',
    templateUrl: './socialContacts.component.html',
    styleUrls: ['./socialContacts.component.scss']
})
export class AdminSocialContactComponent extends DataTableComponentBase {
    ...
    ngOnInit() {
        ...
        initDataTableInput.ColumnDefs = [
            ...
            {
                title: this.l('Common.SocialName'), searchable: true, name: "socialName", data: 'socialName', targets: 1, orderable: true,
                render: (data, type, full) => {
                    return data || '';
                }
            },
            {
                title: this.l('Common.SocialValue'), searchable: true, name: "socialValue", data: 'socialValue', targets: 2, orderable: true,
                render: (data, type, full) => {
                    return data || '';
                }
            },
            ...
            },
            {
                ...
                render: (data, type, full) => {
                    if (this.isDeleted == false) {
                        let rowDiv = this.isGranted("Write.Permission.ContactUs") ?'<button type="button" class="btn btn-round btn-primary buttons-html5 edit"><i class="fa fa-edit"></i></button>' : "";
        ...
        initDataTableInput.DataTableSrc = "/api/services/app/SocialContact/IsPaged";
        this.columnDefs = super.initDataTable(initDataTableInput);

    }

    protected edit?(obj: SocialContactDto): void {
        this.selectedObj = obj;
        this.openModal();
    }

    addNew() {
        this.selectedObj = new SocialContactDto();
        this.openModal();
    }
    

    openModal() {
        this.dialogService.open(DialogManageSocialContactComponent, {
            context: {
