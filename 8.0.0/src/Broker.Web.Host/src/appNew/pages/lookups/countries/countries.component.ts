...
import { DialogManageCountryComponent } from './dialog-manage-country.component';
...
export class AdminCountriesComponent extends DataTableComponentBase {
    ...
    columnDefs: any[] = [];
    selectedObj: CountryDto = new CountryDto();
    searchValue: any;
            ...
            },
            {
                ...
                render: (data, type, full) => {
                    if (this.isDeleted == false) {
                        let rowDiv = /*this.isGranted("Write.Permission.Countries") ?*/'<button type="button" class="btn btn-round btn-primary buttons-html5 edit"><i class="fa fa-edit"></i></button>' /*: ""*/;
                        rowDiv += /*this.isGranted("Delete.Permission.Countries") ?*/'<a href="javascript:void(0);" class="btn btn-icon btn-sm mx-3 btn-danger delete confirm" data-action="Delete" data-id="' + data + '" title="' + this.l("Common.Delete") + '"><i class="fa fa-trash"></i></a>' /*: ""*/;
                        if (full.isEnabled) {
                            rowDiv += this.isGranted("Write.Permission.Countries") ? '<a href="javascript:void(0);" class="btn btn-icon btn-sm mx-3 btn-danger disable confirm" data-action="Disable" data-id="' + data + '" title = "' + this.l("Common.Disable") + '"> <i class="fa fa-times"></i></a>' : '';
                        }
                        else {
                            rowDiv += this.isGranted("Write.Permission.Countries") ? '<a href="javascript:void(0);" class="btn btn-icon btn-sm mx-3 btn-success enable confirm" data-action="Enable" data-id="' + data + '" title = "' + this.l("Common.Enable") + '"> <i class="fa fa-check"></i></a>' : '';
                        }
                        return rowDiv;
                    }
                    else {
                        return /*this.isGranted("Delete.Permission.Countries") ?*/ '<a href="javascript:void(0);" class="btn btn-icon btn-sm mx-3 btn-danger delete confirm" data-action="Restore" data-id="' + data + '" title="' + this.l("Common.Restore") + '"><i class="fa fa-undo"></i></a>' /*: ""*/;
                    }
                }
            }
        ];

        initDataTableInput.DataTableSrc = "/api/services/app/Country/IsPaged";
        this.columnDefs = super.initDataTable(initDataTableInput);

    }

    protected edit?(obj: CountryDto): void {
        this.selectedObj = obj;
        this.openModal();
    }

    addNew() {
        this.selectedObj = new CountryDto();
        this.openModal();
    }

    openModal() {
        this.dialogService.open(DialogManageCountryComponent, {
            context: {
                countryId: this.selectedObj.id,
                countryNameAr: this.selectedObj.nameAr,
                countryNameEn: this.selectedObj.nameEn
            },
