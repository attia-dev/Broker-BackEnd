...
import { DialogManageCityComponent } from './dialog-manage-city.component';
...
export class AdminCitiesComponent extends DataTableComponentBase {
    protected rejectResonBalance?(obj: object): void {
        throw new Error('Method not implemented.');
    }
    public closeResult: string;
    columnDefs: any[] = [];
    selectedObj: CityDto = new CityDto();
    governorate: GovernorateDto = new GovernorateDto();
    ...
    ngOnInit() {
        ...
        initDataTableInput.ColumnDefs = [
            ...
            {
                title: this.l('Common.Governorate'), searchable: true, name: "governorate.nameEn", data: this.localization.currentLanguage.name != 'en' ? 'governorate.nameAr' : 'governorate.nameEn', targets: 3, orderable: true,
                render: (data, type, full) => {
                    return data || '';
                }
            },
            ...
            },
            {
                ...
                render: (data, type, full) => {
                    if (this.isDeleted == false) {
                        ...
                        if (full.isEnabled) {
                            rowDiv += this.isGranted("Write.Permission.Cities") ? '<a href="javascript:void(0);" class="btn btn-icon btn-sm mx-3 btn-danger disable confirm" data-action="Disable" data-id="' + data + '" title = "' + this.l("Common.Disable") + '"> <i class="fa fa-times"></i></a>' : '';
                        }
                        else {
                            rowDiv += this.isGranted("Write.Permission.Cities") ? '<a href="javascript:void(0);" class="btn btn-icon btn-sm mx-3 btn-success enable confirm" data-action="Enable" data-id="' + data + '" title = "' + this.l("Common.Enable") + '"> <i class="fa fa-check"></i></a>' : '';
                        }
                        return rowDiv;
                    }
                    else {
        ...
        initDataTableInput.DataTableSrc = "/api/services/app/City/IsPaged";
        this.columnDefs = super.initDataTable(initDataTableInput);

    }

    protected edit?(obj: CityDto): void {
        debugger;
        this.selectedObj = obj;
        this.selectCountry = [];
        const z = new CountryDto();
        z.id = this.selectedObj.governorate.countryId;
        z.nameEn = this.selectedObj.governorate.country.nameEn;
        z.nameAr = this.selectedObj.governorate.country.nameAr;
        this.selectCountry.push(z);


        this.selectGovernorate = [];
        const x = new GovernorateDto();
        x.id = this.selectedObj.governorateId;
        x.nameEn = this.selectedObj.governorate.nameEn;
        x.nameAr = this.selectedObj.governorate.nameAr;
        this.selectGovernorate.push(x);
    ...
    addNew() {
        this.selectCountry = [];
        this.selectGovernorate = [];
        this.selectedObj = new CityDto();
        this.openModal();
    }

    openModal() {
        debugger;
        this.dialogService.open(DialogManageCityComponent, {
            context: {
                cityId: this.selectedObj.id,
                cityNameAr: this.selectedObj.nameAr,
                cityNameEn: this.selectedObj.nameEn,
