...
import { DialogManageAdvertisementRejectReasonComponent } from './dialog-manage-advertisementRejectReason.component';
...
export class AdvertisementsComponent extends DataTableComponentBase {
    ...
    ngOnInit() {
                ...
                render: (data, type, full) => {
                    if (this.isDeleted == false) {
                        ...
                        if (full.isApprove != null) {
                            if (full.isApprove) {
                                rowDiv += this.isGranted("Write.Permission.Advertisements") ? '<a href="javascript:void(0);" class="btn btn-icon btn-sm mx-3 btn-danger rejectResonBalance confirm" data-action="Decline" data-id="' + data + '" title = "' + this.l("Common.Reject") + '"> <i class="fa fa-times"></i></a>' : '';
                              //  rowDiv += this.isGranted("Write.Permission.Advertisements") ? '<a href="javascript:void(0);" class="btn btn-icon btn-sm mx-3 btn-danger rejectResonBalance confirm" data-action="Decline" data-id="' + data + '" title = "' + this.l("Common.Reject") + '"> <i class="fa fa-times"></i></a>' : "";
                            }
                            else {
        ...
        //this.setAjaxParam("IsEdited", undefined);
    ...
    openReject() {
        this.dialogService.open(DialogManageAdvertisementRejectReasonComponent, {
            context: {
    ...
    searchName(value, IsApproved,Newer) {
        ...
        if (this.request != null) {
            ...
            this.addAjaxParam("IsEdited", Newer);
    ...
    clearSearch() {
        ...
        this.request.IsEdited = undefined;
        this.clearAjaxParams();
        this.rerender();
    }
