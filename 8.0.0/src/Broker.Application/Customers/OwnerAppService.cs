...
{
    public class OwnerAppService : BrokerAppServiceBase, IOwnerAppService
    {
        ...
        public OwnerAppService( IRepository<Owner, long> ownerRepository, UserManager userManager, IUserAppService userAppService,
        ...
        public async Task<DataTableOutputDto<OwnerDto>> IsPaged(GetOwnerInput input)
        {
            try
            {
                using (CurrentUnitOfWork.DisableFilter(AbpDataFilters.SoftDelete))
                {
                    if ((input.action == "Delete" || input.action == "Restore"))
                    {
                        if (input.actionType == "GroupAction")
                        {
                            ...
                            {
                                var brokerPerson = await _ownerRepository.FirstOrDefaultAsync(Convert.ToInt32(input.ids[i]));
                                if (brokerPerson != null)
                                {
                                    if (input.action == "Delete")
                                    {
                                        //await _ownerRepository.DeleteAsync(brokerPerson);
                                        #region HardDelete

                                        brokerPerson.IsDeleted = true;
                                        await _ownerRepository.UpdateAsync(brokerPerson);
                                        ...
                                        var advertisments = _advertisementRepository.GetAll().Where(x => x.OwnerId == brokerPerson.Id);

                                        foreach (var advertisment in advertisments)
                                        {
                                            advertisment.OwnerId = null;
                        ...
                        else if (input.actionType == "SingleAction")
                        {
                            ...
                            {
                                var brokerPerson = await _ownerRepository.FirstOrDefaultAsync(Convert.ToInt32(input.ids[0]));
                                if (brokerPerson != null)
                                {
                    ...
                    var query = _ownerRepository.GetAllIncluding(x => x.User).Where(x => x.User.PhoneNumber != "Deleted@Deleted.Deleted"); 
                    ...
                    var owners = await query
                        .Include(x => x.CreatorUser).Include(x => x.LastModifierUser).Include(x => x.DeleterUser)
                          ...
                          .Take(input.length)
                          .ToListAsync();
                    return new DataTableOutputDto<OwnerDto>
                    {
                        draw = input.draw,
                        iTotalDisplayRecords = filteredCount,
                        iTotalRecords = count,
                        aaData = ObjectMapper.Map<List<OwnerDto>>(owners),
                    };
                }
            }
            catch (Exception ex)
        ...
        public async Task<OwnerDto> Manage(OwnerDto input)
        {
            try
            {
                if (input.Id > 0)
                {
                    ...
                    owner.Avatar = input.Avatar;
                    owner.SecondMobile = input.SecondMobile;
                    owner.IsActive = input.IsActive;

                    if (owner.PackageId == null || owner.PackageId != input.PackageId)
                    {
                        var package = await _PackageAppService.GetById(new EntityDto<long>((long)input.PackageId));
                        if (package != null)
                            owner.Balance = package.Points;
                    }
                    else
                        owner.Balance = input.Balance;

                    owner.PackageId = input.PackageId;

                    var updatedOwner = await _ownerRepository.UpdateAsync(owner);
                    await CurrentUnitOfWork.SaveChangesAsync();
                    var user = new User();
                    //CurrentUnitOfWork.EnableFilter(AbpDataFilters.MayHaveTenant);
                    user = await _userManager.FindByIdAsync(updatedOwner.UserId.ToString());
                    ...
                    return ObjectMapper.Map<OwnerDto>(owner);
                }
                else
                {
                    //User user = new User();
                    ...
                    await CurrentUnitOfWork.SaveChangesAsync();
                    var owner = Owner.Create(user1.Id, input.SecondMobile, input.Avatar, input.IsActive);
                    var res = await _ownerRepository.InsertAsync(owner);
                    await CurrentUnitOfWork.SaveChangesAsync();
                    return ObjectMapper.Map<OwnerDto>(res);
                }
            }
            catch (Exception ex)
            {
        ...
        public async Task<OwnerDto> GetById(EntityDto<long> input)
        {
            try
            {
                ...
                if (input.Id > 0)
                {
                    var owner = await _ownerRepository.GetAllIncluding(x => x.User).Where(x => x.Id == input.Id).FirstOrDefaultAsync();
                    if (owner != null)
                    {
                        ownerinfo = ObjectMapper.Map<OwnerDto>(owner);
                    }
                }
                return ownerinfo;
            }
        ...
        public async Task<OwnerDto> GetByUserId(EntityDto<long> input)
        {
            try
            {
                OwnerDto ownerinfo = new OwnerDto();
                if (input.Id > 0)
                {
                    var owner = await _ownerRepository.GetAllIncluding(x => x.User).Where(x => x.UserId == input.Id).FirstOrDefaultAsync();
                    if (owner != null)
                    {
                        ownerinfo = ObjectMapper.Map<OwnerDto>(owner);
                    }
        ...
        public async Task<GetOwnerOutput> GetAll(PagedOwnerResultRequestDto input)
        {
            try
            {
                var query = _ownerRepository.GetAll();
                //var query = _ownerRepository.GetAllIncluding(x => x.User).Where(x => x.IsDeleted == false);
                ...
                return new GetOwnerOutput { Owners = ObjectMapper.Map<List<OwnerDto>>(list) };
            }
            catch (Exception ex)
            {
                throw ex;
        ...
        public async Task<OwnerDto> UpdateOwnerFromMobile(OwnerDto input)
        {
            try
            {
                var owner = await _ownerRepository.GetAsync(input.Id);
